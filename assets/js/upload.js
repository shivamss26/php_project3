...
jQuery(function($) {
    ...
    jQuery(document).on('click', 'input.select-img', function(event) {
        ...
        var Instant_News_Image = wp.media.controller.Library.extend({
            defaults: _.defaults({
        ...
        instant_news_file_frame = wp.media.frames.instant_news_file_frame = wp.media({
            button: {
                text: jQuery(this).data('uploader_button_text'),
            },
            state: 'author-insert-image',
            states: [
                new Instant_News_Image()
            ],
            multiple: false // Set to true to allow multiple files to be selected
        });

        // When an image is selected, run a callback.
        instant_news_file_frame.on('select', function() {
            var state = instant_news_file_frame.state('author-insert-image');
